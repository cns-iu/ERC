...
visualizationFunctions.HVBarChart = function(element, data, opts) {
    ...
    this.VisFunc = function() {
        ...
        if (chartOrientation == "vertical") {
            var newHeight = (22 * context.filteredData[context.PrimaryDataAttr].data.length + 100)
            if (context.config.dims.fixedHeight < newHeight) {
                $(element[0]).parent().css({height: newHeight});
            }
        ...
        function createBars() {
            return context.SVG.visG.append("g")
                ...
                .each(function(d, i) {
                    ...
                    var prevX = 0;
                    var prevY = 0;
                    d[context.config.meta.nodes.rowAggregator].forEach(function(d1, i1) {
                        if (chartOrientation == "horizontal") {
